...
// Backs login rate-limiting. Previously an in-memory Map, which doesn't
// survive across serverless invocations/instances (e.g. on Vercel) - the DB
// is the only state that's actually shared across every request there.
export const loginAttempts = pgTable('login_attempts', {
  rateLimitKey: text('rate_limit_key').primaryKey(), // `${ip}_${identifier}`
  count: integer('count').default(0).notNull(),
  firstAttempt: timestamp('first_attempt').defaultNow().notNull(),
  lockedUntil: timestamp('locked_until'),
});

// Backs password-reset OTPs. Same reasoning as loginAttempts above.
